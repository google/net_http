...
namespace net_http
{
                                          ...
                                          const char **header_end)
  {
    ...
    while (pos < end)
    {
      ...
      {
      case IN_HEADER_ID1:
        if (*pos != kMagicHeader[0])
          return INVALID_HEADER;
        pos++;
        state_++;
        break;
      case IN_HEADER_ID2:
        if (*pos != kMagicHeader[1])
          return INVALID_HEADER;
        pos++;
        state_++;
        break;
  ...
  {
    if (comp_stream_.avail_out < sizeof(GZIP_HEADER))
      return Z_BUF_ERROR;
  ...
  { // Z_SYNC_FLUSH or Z_FINISH
    ...
    *sourceLen = uncomp_stream_.avail_in;
    ...
    if (err == Z_STREAM_END && !settings_.dont_hide_zstream_end_)
      err = Z_OK;
