...
goog.module('goog.labs.net.webChannel.ChannelRequest');
...
/** @suppress {extraRequire} unused, try to remove this. */
 ...
 * @param {WebChannelDebug} channelDebug A
...
ChannelRequest.prototype.sendXmlHttp_ = function(hostPrefix) {
  ...
  const headers =
      this.extraHeaders_ ? googObject.clone(this.extraHeaders_) : {};
  if (this.postData_) {
    if (!this.verb_) {
      this.verb_ = 'POST';
    }
...
ChannelRequest.prototype.readyStateChangeHandler_ = function(evt) {
  ...
  if (throttle && xhr.getReadyState() == XmlHttp.ReadyState.INTERACTIVE) {
...
ChannelRequest.prototype.decodeNextChunks_ = function(
  ...
  if (readyState == XmlHttp.ReadyState.COMPLETE && responseText.length == 0 &&
...
ChannelRequest.prototype.onWatchDogTimeout_ = function() {
  this.watchDogTimerId_ = null;
  const now = Date.now();
  asserts.assert(this.watchDogTimeoutTime_, 'WatchDog timeout time missing?');
