...
goog.module('goog.labs.net.webChannel.WebChannelBase');
...
const ConnectionState = goog.require('goog.labs.net.webChannel.ConnectionState');
 ...
 * @implements {Channel}
 */
function WebChannelBase(opt_options, opt_clientVersion, opt_conn) {
  /**
   ...
   * @private @const {!XmlHttpFactory | undefined}
   */
  this.xmlHttpFactory_ =
      (opt_options && opt_options.xmlHttpFactory) || undefined;

  /**
   * The server URL to use for test network availability.
   * @private @const {string|undefined}
   */
  this.networkTestUrl_ =
      (opt_options && opt_options.networkTestUrl) || undefined;
   ...
   * @private @const {!ForwardChannelRequestPool}
   */
  this.forwardChannelRequestPool_ = new ForwardChannelRequestPool(
      opt_options && opt_options.concurrentRequestLimit);

  /**
   * The maximum number of maps that can be sent in one POST.
   *
   * Defaults to 1000 and cannot exceed 1000.
   * @private @const {number}
   */
  this.maxMapsPerRequest_ = Math.min(
      (opt_options && opt_options.maxMessagesPerRequest) || 1000, 1000);

  /**
   * The V8 codec.
   * @private @const {!WireV8}
   */
  ...
  /**
   * TODO(wenboz): move all backchannel states to its own class similar to
...
WebChannelBase.prototype.sendMap = function(map, opt_context) {
  ...
  if (this.outgoingMaps_.length == this.maxMapsPerRequest_) {
    ...
    this.channelDebug_.severe(() => {
      return 'Already have ' + this.maxMapsPerRequest_ +
          ' queued maps upon queueing ' + googJson.serialize(map);
    });
  }
...
WebChannelBase.prototype.open_ = function() {
  ...
  if (this.initHeaders_) {
    if (extraHeaders) {
      extraHeaders = googObject.clone(extraHeaders);
      googObject.extend(extraHeaders, this.initHeaders_);
    } else {
 ...
 * encoded as raw JSON data, return this.maxMapsPerRequest_
 * @private
 */
...
WebChannelBase.prototype.makeForwardChannelRequest_ = function(
  ...
  requestText = this.dequeueOutgoingMaps_(request, this.maxMapsPerRequest_);
...
WebChannelBase.prototype.addAdditionalParams_ = function(uri) {
  if (this.extraParams_) {
    googObject.forEach(this.extraParams_, function(value, key) {
  ...
  if (this.handler_) {
    ...
    if (params) {
      googObject.forEach(params, function(value, key) {
        uri.setParameterValue(key, value);
      });
    }
  }
...
WebChannelBase.prototype.onStartBackChannelTimer_ = function() {
  ...
  // Note that the timeout factor only affects the false-positive rate.
  const bpDetectionTimeout = 4 * this.handshakeRttMs_;
...
WebChannelBase.prototype.onInput_ = function(respArray, request) {
  ...
  for (let i = 0; i < respArray.length; i++) {
    ...
    const incomingArrayId = nextArray[0];
    // Skip the array if it is older than the last array received. This can
    // happen if the backend ignores the AID parameter in the request and sends
    // messages that have been previously sent.
    if (incomingArrayId <= this.lastArrayId_) {
      this.channelDebug_.warning(
          'Ignoring out-of-order or duplicate message with arrayId: ' +
          incomingArrayId + ', lastArrayId: ' + this.lastArrayId_);
      continue;
    } else if (
        incomingArrayId > this.lastArrayId_ + 1 && this.lastArrayId_ > -1) {
      this.channelDebug_.warning(
          'Received non-consecutive message with arrayId: ' + incomingArrayId +
          ', lastArrayId: ' + this.lastArrayId_);
      // TODO: The loss of messages is not recoverable by the client, we may
      // consider closing the channel immediately.
    }
    this.lastArrayId_ = incomingArrayId;
...
WebChannelBase.prototype.signalError_ = function(error) {
  ...
  if (error == WebChannelBase.Error.REQUEST_FAILED) {
    ...
    netUtils.testNetwork(
        goog.bind(this.testNetworkCallback_, this), this.networkTestUrl_);
  } else {
...
/**
 * @return {number} The number of messages in flight, from the oldest message
 * not yet acknowledged by the server to the next message to be sent out.
 */
WebChannelBase.prototype.getNonAckedMessageGap = function() {
  const nonAckedMaps = this.getNonAckedMaps();
  if (nonAckedMaps.length == 0) {
    return 0;
  }
  let minMapId = nonAckedMaps[0].mapId;
  for (let i = 1; i < nonAckedMaps.length; i++) {
    minMapId = Math.min(minMapId, nonAckedMaps[i].mapId);
  }
  return this.nextMapId_ - minMapId;
};
...
WebChannelBase.prototype.createXhrIo = function(hostPrefix, isStreaming) {
  ...
  if (this.usesFetchStreams_ && !this.xmlHttpFactory_) {
    xhr = new XhrIo(new FetchXmlHttpFactory({streamBinaryChunks: isStreaming}));
  } else {
