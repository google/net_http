...
- (void)logHTTPRequest:(NSString *)verb
  ...
  if (!gWCVerboseLoggingEnabled) {
    // Avoid string formatting if verbose logging is disabled.
    return;
  }
  NSString *text = [NSString stringWithFormat:@"XMLHTTP REQ (%@) [attempt %lld ]: %@ \n %@ \n %@",
                                              ID, attempt, verb, URL.absoluteString, postData];
  [self logDebug:text];
}
...
- (void)logHTTPChannelResponseMetaData:(NSString *)verb
  ...
  NSString *text = [NSString
      stringWithFormat:@"XMLHTTP RESP (%@) [attempt %lld ]: %@ \n %@ \n %@ %d", ID, attempt, verb,
                       URL.absoluteString, [self formatRequestReadyStateToString:state], code];
  [self logDebug:text];
}
