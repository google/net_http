...
void H2Client::ReadCallback(bufferevent* bev,
  ...
  ssize_t recv_len = nghttp2_session_mem_recv(sess->h2session,
                                              ...
                                              len);
  if (recv_len < 0) {
    VLOG(1) << "nghttp2_session_mem_recv() failed: "
  ...
  if (send_rv < 0) {
    VLOG(1) << "nghttp2_session_send() failed: "
  ...
  if (what & BEV_EVENT_CONNECTED) {
    ...
    if (fd >= 0) {
      ...
      int rv = setsockopt(fd,
                          ...
                          sizeof(one));
      if (rv != 0) {
        VLOG(1) << "H2Client: setsockopt(TCP_NODELAY) failed";
      }
    }
...
void H2Client::HandleSessionError(Session* sess) {
  if (sess->web_stream) {
    sess->web_stream->OnError();
    delete sess->web_stream;
    sess->web_stream = nullptr;
...
nghttp2_ssize H2Client::SendCallback(nghttp2_session* /*session*/,
                                     const uint8_t* data,
                                     size_t length,
                                     int /*flags*/,
                                     void* user_data) {
...
int H2Client::OnFrameRecvCallback(nghttp2_session* /*session*/,
  ...
  if (frame->hd.type == NGHTTP2_HEADERS &&
    ...
    if (sess->response_status == 200) {
      ...
      VLOG(2) << "H2Client: Server rejected stream with status: "
...
int H2Client::OnStreamCloseCallback(nghttp2_session* /*session*/,
  ...
  if (sess->web_stream && stream_id == sess->h2_stream_id) {
    if (error_code != NGHTTP2_NO_ERROR) {
      VLOG(2) << "H2Client: Stream closed with error code: " << error_code;
...
// that the stream object can be created after nghttp2_submit_request2 returns
// the real stream_id (the id is not known before that call).
nghttp2_ssize H2Client::DataSourceReadCallback(nghttp2_session* session,
                                               int32_t stream_id,
                                               uint8_t* buf,
                                               size_t length,
                                               uint32_t* data_flags,
...
void H2Client::InitH2Session(Session* sess) {
  ...
  int32_t stream_id = nghttp2_submit_request2(sess->h2session,
                                              ...
                                              nullptr);
  if (stream_id < 0) {
    VLOG(1) << "H2Client: nghttp2_submit_request2() failed: "
  ...
  int send_rv = nghttp2_session_send(sess->h2session);
  if (send_rv < 0) {
    VLOG(1) << "H2Client: nghttp2_session_send() failed: "
            << nghttp2_strerror(send_rv);
